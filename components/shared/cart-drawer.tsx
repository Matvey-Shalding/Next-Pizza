				...
				{open && (
					...
					>
						<motion.div
							...
							className={cn({'h-screen bg-[#F4F1EE] flex flex-col gap-y-5 pt-5' : items.length > 0,'bg-white grid place-content-center' : items.length === 0})}
						>
							{items.length > 0 ? 							<><div className='text-lg text-black flex px-5 items-center justify-between w-full'>
								<span>
							...
							<span className='text-gray-400 text-center inline-block mb-5 max-w-50'>Add at least one pizza to make an order</span>
							<Button className='flex min-w-60 min-h-[50px] items-center gap-x-1.5' onClick={() => setOpen(false)}>
